...
DO $$
BEGIN
    IF NOT EXISTS (SELECT FROM pg_database WHERE datname = 'hospital_billing_db') THEN
...
CREATE TABLE IF NOT EXISTS bills (
    ...
    CONSTRAINT chk_due_vs_total CHECK (due_amount = total_amount - paid_amount)
);

-- Indexes for bills (only what your APIs actually need)
...
-- Indexes for payments (purposeful only)
